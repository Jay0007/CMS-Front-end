      ...
      <v-card align="center">
        ...
        >
          <v-card class="login_area" flat>
            <div class="display-1 text--primary"><v-icon>mdi-account</v-icon>Log in</div>
          ...
          <v-card elevation='0' outlined>
            <v-card-text>
              <v-form v-model="validx" @keyup.native.enter="login">
                ...
                <v-btn @click="login" :disabled="!validx" :loading="loading" class="ma-0 white--text px-4 font-weight-bold" style="background:#f4a631;border-radius:25px;font-size:0.9em">LOGIN</v-btn>
              </v-form>
            </v-card-text>
          </v-card>
        </v-col>
...
export default {
  ...
  methods: {
    async login(){
      ...
      var that = this;
      await axios
        .post(this.$store.getters.getBaseUrl + "/api/user/login", userData)
        .then(function (res) {
          that.loading = false;
